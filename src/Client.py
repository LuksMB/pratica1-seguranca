...
signed_message = create_signed_message(A, private_key_bytes, GITHUB_USERNAME) # Cria a mensagem assinada com a chave privada Ed25519
...
try:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        ...
        server_dh_public_key, signature, server_username = read_signed_message(dh_handshake_data)  # Lê a mensagem assinada recebida do servidor, extraindo a chave pública Diffie-Hellman do servidor

        chaves = baixar_chaves_publicas_github(server_username)
        if not chaves:
        ...
        if verificar_assinatura(chaves, server_dh_public_key, signature, server_username):
